...
async def upload_local_inputs(
    ...
    """
    activity.logger.info(f"Uploading {len(inputs)} local input(s) to {target_server}")

    uploaded_files = {}
    client = ComfyUIClient(target_server)
    ...
    try:
        for input_key, input_def in inputs.items():
            ...
            activity.logger.info(
                f"Uploading '{input_key}': {source_path} -> {target_filename} ({len(file_data)} bytes)"
            )
            ...
            activity.logger.info(f"Uploaded '{input_key}' as {target_filename}")

    finally:
        await client.close()

    activity.logger.info(f"Successfully uploaded {len(uploaded_files)} file(s)")
