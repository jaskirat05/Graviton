...
def _format_validation_error(error_type: str, error_message: str, node_errors: dict) -> str:
    """Format ComfyUI validation error for display"""
    lines = []
    lines.append(f"[red bold]{error_type}[/red bold]")
    lines.append(f"[red]{error_message}[/red]")

    if node_errors:
        lines.append("")
        lines.append("[dim]Node Errors:[/dim]")
        for node_id, node_err in node_errors.items():
            errors = node_err.get("errors", [])
            class_type = node_err.get("class_type", "Unknown")
            lines.append(f"  [cyan]Node {node_id}[/cyan] ({class_type}):")
            for err in errors[:3]:  # Limit to first 3 errors per node
                err_type = err.get("type", "error")
                err_msg = err.get("message", "")
                err_details = err.get("details", "")
                extra_info = err.get("extra_info", {})

                # Prefer details over generic message
                display_msg = err_details if err_details else err_msg
                if not display_msg:
                    display_msg = str(err)

                # Truncate long messages
                if len(display_msg) > 150:
                    display_msg = display_msg[:150] + "..."

                lines.append(f"    [red]• {display_msg}[/red]")

                # Show extra info if present (e.g., input_name)
                if extra_info:
...
class ChainProgressTracker:
    ...
    def __init__(self, total_steps: int, step_ids: list[str] = None):
        ...
        # Step execution details (for display below progress bar)
        self.step_status: Optional[str] = None  # Current step status line
        self.step_error: Optional[str] = None  # Error message if step failed

    def step_completed(self, step_id: str):
        self.completed_steps.add(step_id)
        self.step_status = None  # Clear step status on completion
    ...
    def step_waiting(self, step_id: str):
        ...
        self.step_status = f"Waiting for approval: {step_id}"
        self.events.append(f"[yellow]⏳[/yellow] Step [cyan]{step_id}[/cyan] waiting for approval")

    def step_executing(self, step_id: str, workflow: str, server: str):
        """Track step starting execution"""
        self.step_status = f"Executing {step_id}: {workflow} on {server}"
        self.step_error = None

    def step_node(self, step_id: str, node_id: str, progress: float = None):
        """Track node execution within a step"""
        progress_str = f" ({int(progress * 100)}%)" if progress else ""
        self.step_status = f"[{step_id}] Node: {node_id}{progress_str}"

    def step_workflow_complete(self, step_id: str, output_count: int = 0):
        """Track step workflow completion (before approval if needed)"""
        self.step_status = f"[{step_id}] Workflow done, {output_count} output(s)"

    def step_workflow_failed(self, step_id: str, error: str):
        """Track step workflow failure"""
        self.step_status = f"[{step_id}] Failed"
        self.step_error = error

    def step_validation_failed(self, step_id: str, error_type: str, error_message: str, node_errors: dict = None):
        """Track step validation failure (ComfyUI rejected the prompt)"""
        self.step_status = f"[{step_id}] Validation Failed"
        # Build detailed error message
        error_parts = [f"{error_type}: {error_message}"]
        if node_errors:
            for node_id, node_err in node_errors.items():
                errors = node_err.get("errors", [])
                class_type = node_err.get("class_type", "Unknown")
                for err in errors[:2]:  # Limit to first 2 errors per node
                    # Prefer details over generic message
                    err_details = err.get("details", "")
                    err_msg = err.get("message", "")
                    display_msg = err_details if err_details else err_msg
                    if not display_msg:
                        display_msg = str(err)
                    error_parts.append(f"  Node {node_id} ({class_type}): {display_msg}")
        self.step_error = "\n".join(error_parts)
    ...
    def render(self) -> Panel:
        """Render the progress display with step details"""
        ...
        # Main progress line
        progress_line = f"{status_icon} {status_text}  [cyan]{bar}[/cyan]  {completed}/{total} steps ({pct}%)"
        lines.append(progress_line)

        # Step status line (light grey)
        if self.step_status:
            lines.append(f"[dim]{self.step_status}[/dim]")

        # Error line (red)
        if self.step_error:
            # Truncate long errors
            error_display = self.step_error[:80] + "..." if len(self.step_error) > 80 else self.step_error
            lines.append(f"[red]{error_display}[/red]")
                ...
                async with httpx_sse.aconnect_sse(http, "GET", url) as sse:
                    async for event in sse.aiter_sse():
                        ...
                        if event_type == "approval_requested":
                            ...
                            # Step workflow started executing
                            step_id = data.get("step_id", "unknown")
                            workflow = data.get("workflow", "unknown")
                            server = data.get("server", "unknown")
                            tracker.step_executing(step_id, workflow, server)
                            live.update(tracker.render())

                        elif event_type == "step_node":
                            # Node executing within step
                            step_id = data.get("step_id", "unknown")
                            node_id = data.get("node_id", "unknown")
                            progress = data.get("progress")
                            tracker.step_node(step_id, node_id, progress)
                            live.update(tracker.render())

                        elif event_type == "step_workflow_complete":
                            # Step workflow finished (before approval check)
                            step_id = data.get("step_id", "unknown")
                            output_count = data.get("output_count", 0)
                            tracker.step_workflow_complete(step_id, output_count)
                            live.update(tracker.render())

                        elif event_type == "step_workflow_failed":
                            # Step workflow failed - show error in red
                            step_id = data.get("step_id", "unknown")
                            error = data.get("error", "Unknown error")
                            tracker.step_workflow_failed(step_id, error)
                            live.update(tracker.render())
                            # Also print error above progress bar
                            live.console.print(f"[red]✗[/red] Step [cyan]{step_id}[/cyan] failed: [red]{error}[/red]")

                        elif event_type == "step_validation_failed":
                            # ComfyUI prompt validation failed - show detailed error
                            step_id = data.get("step_id", "unknown")
                            error_type = data.get("error_type", "validation_error")
                            error_message = data.get("error_message", "Unknown validation error")
                            node_errors = data.get("node_errors", {})

                            tracker.step_validation_failed(step_id, error_type, error_message, node_errors)
                            live.update(tracker.render())

                            # Print detailed validation error above progress bar
                            live.console.print()
                            live.console.print(Panel(
                                _format_validation_error(error_type, error_message, node_errors),
                                title=f"[red bold]Prompt Validation Failed: {step_id}[/red bold]",
                                border_style="red",
                                padding=(0, 1),
                            ))
