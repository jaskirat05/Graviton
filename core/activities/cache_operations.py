...
def _get_log_func(chain_name: Optional[str], chain_version: int, chain_id: Optional[str]):
    """Helper to get a logging function for cache activities."""
    chain_logger = None
...
async def build_cache_from_database(
    ...
    log(f"Building cache for chain: {chain_name}")

    cache = _build_cache(chain_name, exclude_step_ids)

    log(f"Built cache with {len(cache)} steps: {list(cache.keys())}")
    return cache


@activity.defn
async def get_next_chain_version(
    chain_name: str,
    chain_version: int = 1,
    chain_id: Optional[str] = None,
) -> int:
    ...
    with get_session() as db:
        ...
        log(f"Next version for chain '{chain_name}': {next_version}")
        return next_version
